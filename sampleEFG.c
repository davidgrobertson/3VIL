/*   === sampleEFG.c ===
 *
 *   Sample code for calculation of the EFG subset of 3-loop vacuum
 *   basis integrals with (u, v, w, x, y) = (t, t, b, h, W) at the
 *   renormalization scale Q = 150 GeV.
 *
 *   To compile:
 *
 *   gcc -o sampleEFG sampleEFG.c -L<dir> -l3vil -lm
 *
...
int main (int argc, char *argv[])
{
  ...
  TVIL_REAL I_tth, I_thW, I_tbW, I_bhW, I_ttW, I_tbh;
  /* TVIL_REAL H_ttbhWW; */

  /* Evaluate all integrals for this subset: */

  TVIL_EvaluateEFG (t, t, b, h, W, Q2, &result);

  /* Read the integral results from the result struct into real long
     double variables. For the EFG subset, recall that only functions
     *not* involving the variable z are defined. */

  G_tthbW  = TVIL_CREAL(TVIL_GetFunction (&result, "Gvuxwy"));
  ...
  /* Ivwy is also defined for this subset, but in this case it
     duplicates Iuwy since u = v = t, so we omit it: */
